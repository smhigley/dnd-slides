...
export class Datepicker {
  ...
  render() {
    return <div class={{"datepicker-wrapper": true, "open": this.open}}>
      <div class="field-wrapper">
        ...
        <button class="datepicker-button" aria-label={`open calendar, ${this.formatDate(this.currentDate)}`} aria-expanded={`${this.open}`} onClick={() => this.updateOpen(!this.open)} onKeyDown={this.onKeyDown.bind(this)} ref={(el) => this.buttonRef = el}>
      ...
      <div class="popup">
        <dnd-calendar class="calendar" ref={(el) => this.calendarRef = el} contrast={this.contrast} columnheader={this.columnheader} focusModel={this.focusModel} startDate={this.currentDate} selectedDay={this.currentDate} onDateSelected={this.onDateSelect.bind(this)} />
      </div>
    </div>;
  }
  ...
  private renderInput() {
    return <input type="text" class="datepicker-input" onBlur={(ev) => this.parseInput((ev.target as HTMLInputElement).value)} onKeyDown={this.onKeyDown.bind(this)} ref={(el) => this.inputRef = el} />
  }

  private updateOpen(newOpen) {
    if (!newOpen) {
      this.buttonRef.focus();
      console.log('focus button', this.buttonRef);
    }
    else {
      // hack
      setTimeout(() => {
        (this.calendarRef as any).focusDate();
      }, 10);
    }
    this.open = newOpen;
  }
